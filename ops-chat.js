...
class Chat{
 ...
 async start(){
  ...
  this.$('#oc-search').value=this.o.phone||'';this.$('#oc-search').focus();await this.loadList();
 ...
 renderHead(){const {thread:t,address:a,offer:o}=this.data,c=!a?t.closed_request:null;
  this.$('.oc-head').innerHTML=`<div class="oc-row oc-between"><div class="oc-row"><button class="oc-back" data-act="back">←</button><strong>${esc(t.phone)}</strong></div><div class="oc-row"><button data-act="${c?'rebook':'attach'}">${c?'Новая заявка':a?'Адрес':'Привязать адрес'}</button>${this.admin?'<button data-act="assign">Водитель</button>':''}<button data-act="refresh" aria-label="Обновить переписку">↻</button><button data-act="read" title="Отметить просмотренным">✓</button></div></div><p>${esc(a?.text||c?.text||'Добавьте адрес, чтобы согласовать сбор.')}</p><div>${a?`${this.admin?`<button class="oc-chip" data-act="category" title="Изменить категорию" ${this.sendBusy?'disabled':''}>${esc(sourceLabel(a))} ▾</button>`:`<span class="oc-chip">${esc(sourceLabel(a))}</span>`}`:''}${c?`<span class="oc-chip">Заявка закрыта · ${esc(cancellationReasons[c.reason]||c.reason)}</span>`:a?.collection_start?`<span class="oc-chip ok">Согласовано: ${esc(slotLabel(a))}</span>`:a?.date?`<span class="oc-chip ok">В маршруте: ${esc(a.date)}</span>`:'<span class="oc-chip">Вне маршрута</span>'}${t.opted_out?'<span class="oc-chip warn">SMS отключены клиентом · нужен START</span>':!c&&t.manual_mode?'<span class="oc-chip warn">Ручное согласование</span>':o?.state==='awaiting'?'<span class="oc-chip warn">Ожидаем точный ответ YES</span>':''}</div>${a&&['new','planned'].includes(a.status)&&a.kind!=='bank'?'<button data-act="cancel-request" style="margin-top:10px">Закрыть заявку</button>':''}${c?'<p class="oc-help">История сохранена. Для следующего сбора создайте новую заявку.</p>':''}`;
 }
 ...
 async action(act,b){
  ...
  if(act==='rebook'){const c=this.data?.thread.closed_request;if(c)this.newThread(this.data.thread.phone,c);return;}
 ...
 closeCollection(){
  ...
  this.modal('Закрыть текущую заявку',`<p>${esc(a.text)}</p><p class="oc-help">Закроется только этот сбор. Адрес уйдёт из маршрута, история сохранится. Позже можно создать новую заявку с тем же адресом и номером.</p><label>Причина<select id="oc-cancel-reason">${Object.entries(cancellationReasons).map(([v,n])=>`<option value="${v}">${esc(n)}</option>`).join('')}</select></label><label>Примечание<input id="oc-cancel-note" maxlength="2000"></label><p class="oc-help">Если клиент просит другую дату, оставьте заявку открытой и используйте подбор времени.</p><label><input id="oc-cancel-agreed" type="checkbox">Закрыть именно эту заявку</label>${t.opted_out?'<p class="oc-help">SMS отключены клиентом. Закрытие заявки не отменяет этот запрет.</p>':'<label><input id="oc-cancel-draft" type="checkbox" checked>Подготовить ответ клиенту (отправлю отдельно)</label><p class="oc-help">Снимите галочку, если уже ответили клиенту.</p>'}`,async(w)=>{
   ...
   let result;
   try{
    const {data,error}=await this.sb.rpc('subnex_close_collection',{p_data:{thread_id:thread.id,address_id:address.id,version:address.collection_version,reason,note:w.querySelector('#oc-cancel-note').value,agreed:true}});
    if(error){const code=Object.keys(codes).find(c=>error.message?.includes(c));throw new Error(code?codes[code]:'Не удалось закрыть заявку. Проверьте установку 03_close_collection.sql и обновите переписку.');}
    result=data;
