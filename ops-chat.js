...
(function () {
  ...
  const codes = {
    ACCESS_DENIED: 'Нет доступа. Администратор должен назначить ваш аккаунт и водителя.',
    LOGIN_REQUIRED: 'Войдите в аккаунт заново.',
    NOT_CONFIGURED: 'Сервер ещё не настроен. Проверьте установку по инструкции.',
    UK_MOBILE_REQUIRED: 'Укажите британский мобильный номер, например 07446 932887.',
    ADDRESS_REQUIRED: 'Сначала привяжите адрес сбора к переписке.',
    DRIVER_REQUIRED: 'Назначьте активного водителя.',
    SLOT_CONFLICT: 'Этот промежуток уже занят или предложен другому клиенту. Выберите свободное время.',
    SLOT_IN_PAST: 'Выберите будущую дату и время.',
    ...
    SLOT_INVALID: 'Проверьте дату и время.',
    STALE_ADDRESS: 'Адрес или назначение изменились. Обновите переписку и проверьте время заново.',
    AGREEMENT_REQUIRED: 'Подтвердите, что клиент согласовал дату и время.',
    CONFIRMED_CHANGE_REQUIRES_AGREEMENT: 'Для переноса нужно отдельное согласие клиента.',
    USE_CHAT_TO_RESCHEDULE: 'Подтверждённые дата и интервал сохраняются.',
    ALREADY_SCHEDULED_USE_MANUAL: 'Сбор уже запланирован. Подтверждённое время сохраняется.',
    OPTED_OUT: 'Клиент отказался от SMS. Отправка заблокирована до его START.',
    MESSAGE_LENGTH: 'Введите от 1 до 1000 символов.',
    RATE_LIMIT: 'Достигнут лимит отправки. Попробуйте позже.',
    REQUEST_ID_REUSED: 'Запрос уже использован с другим текстом. Обновите переписку.',
    RECENT_DUPLICATE: 'Такое сообщение уже отправлялось в последнюю минуту.',
    PHONE_HAS_ACTIVE_REQUEST: 'На этот номер уже есть активная заявка. Откройте её переписку.',
    CREATE_AUTH_USER_FIRST: 'Сначала создайте и подтвердите пользователя в Supabase → Authentication → Users.',
    CANNOT_DISABLE_YOURSELF: 'Нельзя отключить или понизить собственный аккаунт.',
    DATABASE_ERROR: 'Сервер не сохранил действие. Обновите экран и проверьте данные.',
    SERVICE_UNAVAILABLE: 'Нет подтверждения от сервера. Обновите историю перед повторной отправкой.',
    NETWORK: 'Нет подтверждения от сервера. Текст сохранён в этом окне; повтор использует тот же номер запроса.',
  };
  Object.assign(codes, {
    CANCELLATION_REQUIRED: 'Подтвердите закрытие именно этой заявки.',
    CANCELLATION_REASON: 'Выберите причину закрытия.',
    REQUEST_ALREADY_FINISHED: 'Эта заявка уже завершена. Обновите переписку.',
  });
  ...
  const cancellationReasons = {
    customer_cancelled: 'Клиент отменил сбор',
    already_collected: 'Уже забрали',
    no_bags: 'Мешков для сбора нет',
  ...
  const staleDayHint = (e, day) => {
    ...
    return (
      ...
      (d.to ? ' «' + d.to + '»' : '') +
      ', обещанное окно которого уже прошло — до ' +
      C.hm(latest) +
  ...
  async function api(sb, action, data = {}) {
    ...
    if (error) {
      ...
      const e = new Error(codes[code] || 'Ошибка запроса: ' + code);
      e.code = code;
      throw e;
    }
    if (result?.error) {
    ...
    a?.collection_start
      ? ukDate(a.collection_start) +
        ...
        (SubnexDispatchCore.isDaySpan(a.collection_start, a.collection_end)
          ? 'в течение дня (окно уйдёт утром)'
  ...
  const statuses = {
    pending_auto: 'Ожидает автоматической отправки',
    auto_skipped: 'Не отправлено — заявка или условия изменились',
    received: 'Получено',
    dispatching: 'Отправка начата — ожидаем статус',
    unknown: 'Результат неизвестен — проверьте Twilio',
    accepted: 'Принято Twilio',
    queued: 'В очереди',
  ...
  class Chat {
    ...
    async start() {
      ...
      this.root.innerHTML =
        (this.inline
          ? ''
          : `<header class="oc-top"><div class="oc-brand">SUBNEX <span class="oc-brand-acc">SMS</span><small>Переписка и сборы</small></div><nav>${this.admin ? '<button data-act="settings">График и доступ</button>' : ''}<button data-act="close">Закрыть ×</button></nav></header>`) +
      ...
      try {
        ...
        this.$('.oc-list').innerHTML =
          list
            ...
            .join('') || '<div class="oc-empty">Переписок по этому фильтру нет.</div>';
    ...
    async select(id) {
      ...
      this.$('.oc-conversation').innerHTML = '<div class="oc-empty">Загрузка переписки…</div>';
      ...
      try {
        ...
        if (reset || !this.$('.oc-messages')) {
          this.$('.oc-conversation').innerHTML =
            '<div class="oc-head"></div><div class="oc-messages" aria-label="История сообщений"></div><div class="oc-compose"></div>';
        ...
        if (['ACCESS_DENIED', 'LOGIN_REQUIRED'].includes(e.code)) {
          ...
          this.threadId = null;
          this.$('.oc-conversation').innerHTML =
            '<div class="oc-empty">Доступ к переписке закрыт. Войдите заново или проверьте назначение.</div>';
        }
        this.notice(errText(e));
      } finally {
        this.detailLoading = false;
    ...
    renderHead() {
      ...
      const slot = a?.collection_start
        ? `<span class="oc-chip ${finished || closed ? '' : 'ok'}">${finished || closed ? 'Было согласовано' : 'Согласовано'}: ${esc(slotLabel(a))}</span>`
        : a?.date
          ? `<span class="oc-chip ${finished || closed ? '' : 'ok'}">${finished || closed ? 'Дата сбора' : 'В маршруте'}: ${esc(a.date)}</span>`
          : !closed
            ? '<span class="oc-chip">Вне маршрута</span>'
            : '';
      const action =
        (a && a.kind !== 'bank') || c
          ? `<button data-act="cancel-request" style="margin-top:10px" ${closed ? 'disabled aria-disabled="true"' : this.sendBusy ? 'disabled' : ''}>${closed ? 'Заявка закрыта' : 'Закрыть заявку'}</button>`
          : '';
    ...
    renderMessages(messages, prepend = false) {
      ...
      const html =
        `${messages.length >= 100 ? '<button data-act="older">Более ранние сообщения</button>' : ''}` +
        messages
          .map(
            (m) =>
    ...
    preview() {
      ...
      if (b && dayOnly) {
        ...
        this.$('#oc-count').textContent = b.value.length + ' / 1000 символов';
        this.$('#oc-preview').textContent = h
          ? 'SMS только с датой. После YES сбор встанет на ' + day + ' без времени; окно прибытия уйдёт утром при старте маршрута.'
          : 'На эту дату нет рабочих часов — выберите другой день.';
        return;
      }
      if (b && this.partnerTemplate()) {
        ...
        this.$('#oc-preview').textContent =
          ...
          '. Дата и часы обновляются автоматически. Подпись: ' +
          (this.driverName() || 'без имени — назначьте водителя') +
          '.';
        return;
      }
      if (b) this.$('#oc-count').textContent = b.value.length + ' / 1000 символов';
    ...
    async action(act, b) {
      ...
      if (act === 'mode') {
        ...
        if (
          ...
          this.notice('Для нового сбора создайте отдельную заявку.');
          return;
        }
        this.mode = mode;
        this.compose();
      ...
      if (act === 'attach') {
        if (this.data.address) {
          this.notice('Адрес: ' + this.data.address.text + '. Редактирование реквизитов доступно в разделе «Адреса».');
   ...
   <textarea id="oc-mv-text" rows="7" readonly></textarea>`,
        async (wrap) => {
          ...
          if (wrap.querySelector('#oc-mv-sms').checked) {
            try {
              ...
              tail = ' Сообщение не ушло: ' + errText(e) + ' Напишите клиенту вручную.';
            }
          }
          await this.loadDetail(true);
          await this.loadList();
        ...
        try {
          ...
          sel.innerHTML = slots.length
            ? slots
                .map(
                  (s, i) =>
                    `<option value="${i}">${esc(s.start)}–${esc(s.end)}${s.extra_minutes ? ' · +' + s.extra_minutes + ' мин дороги' : ''}</option>`,
                )
                .join('')
    ...
    async send() {
      ...
      if (this.mode === 'offer' && this.dayOnly && this.$('#oc-dayonly')) {
        ...
        if (!h) {
          this.notice('На эту дату нет рабочих часов — выберите другой день.');
          return;
        }
      ...
      this.notice(plan ? 'Проверяю маршрут перед отправкой…' : 'Отправка…');
      try {
        if (!this.pending) {
          const extra = plan
        ...
        this.notice(
          r.uncertain
            ...
            : 'Статус: ' + (statuses[r.message.status] || r.message.status),
        );
        this.o.onChanged?.();
      } catch (e) {
        if (
    ...
    async confirm() {
      ...
      if (result) {
        this.notice('Дата и время сохранены. Адрес добавлен в маршрут.');
        await this.loadDetail();
        await this.loadList();
        this.compose();
        this.o.onChanged?.();
    ...
    modal(title, content, onSave, saveLabel = 'Сохранить') {
      ...
      wrap.innerHTML = `<section class="oc-modal" role="dialog" aria-label="${esc(title)}"><h2>${esc(title)}</h2><div class="oc-form">${content}</div><p class="oc-modal-error" role="alert"></p><div class="oc-actions"><button class="oc-cancel">Отмена</button>${onSave ? `<button class="oc-primary oc-save">${esc(saveLabel)}</button>` : ''}</div></section>`;
    ...
    newThread(phone = '', previous = null) {
      ...
      this.modal(
        previous ? 'Новая заявка' : phone ? 'Привязать адрес' : 'Новая переписка',
        `<label>Существующая заявка<select id="oc-existing"><option value="">Новый запрос</option>${addresses.map((a) => `<option value="${esc(a.id)}">${esc(a.text)} · ${esc(a.phone)}</option>`).join('')}</select></label><label>Мобильный телефон<input id="oc-new-phone" type="tel" value="${esc(phone)}" placeholder="07…"></label><label>Адрес нового сбора<input id="oc-new-address" value="${esc(previous?.text || '')}" placeholder="Улица, дом, город, индекс"></label><label>Категория<select id="oc-new-source">${Object.entries(
          sources,
        )
          .map(([v, n]) => `<option value="${v}" ${v === previous?.source ? 'selected' : ''}>${n}</option>`)
          .join(
            '',
          )}</select></label>${this.admin ? `<label>Водитель<select id="oc-new-driver">${this.driverOptions(previous ? this.data.thread.driver_id : this.profile.driver_id)}</select></label>` : ''}<p class="oc-help">Новый адрес останется вне маршрута до согласования срока. Координаты можно уточнить в редакторе адреса.</p>`,
        async (w) => {
          const data = {
    ...
    closeCollection() {
      ...
      if (isClosed(a)) {
        this.notice('Заявка уже закрыта. Для следующего сбора нажмите «Новая заявка».');
        return;
      }
      const address = { ...a },
        thread = { ...t },
        ...
        async (w) => {
          ...
          try {
            ...
            if (error) {
              ...
              throw new Error(code ? codes[code] : 'Не удалось закрыть заявку. Проверьте установку 05_auto_sms.sql и обновите переписку.');
            }
            result = data;
          } finally {
            this.sendBusy = false;
          ...
          if (!this.closed && this.threadId === thread.id) {
            ...
            if (this.data?.thread.closed_request?.id === result.id && !this.data.address) {
              ...
              if (n?.state === 'pending') notice = 'Заявка закрыта. SMS об отмене в очереди на отправку.';
                ...
                notice = 'Заявка закрыта. SMS не отправлена — проверьте номер клиента и текущую заявку.';
              else if (['failed', 'unknown'].includes(n?.state))
                notice = 'Заявка закрыта. Проверьте статус SMS в истории перед повторной отправкой.';
              this.notice(notice);
            }
          }
    ...
    changeCategory() {
      ...
      this.modal(
        'Категория сбора',
        `<label>Категория<select id="oc-category">${Object.entries(sources)
          .map(([v, n]) => `<option value="${v}" ${v === address.collection_source ? 'selected' : ''}>${esc(n)}</option>`)
          .join('')}</select></label><p class="oc-help">Категория определяет шаблон новых SMS. Согласованные дата и время сохраняются.</p>`,
        async (w) => {
          if (this.sendBusy) throw new Error('Дождитесь завершения отправки SMS.');
    ...
    assign() {
      this.modal(
        'Назначить водителя',
        `<label>Водитель<select id="oc-assignee">${this.driverOptions(this.data.thread.driver_id)}</select></label><p class="oc-help">История останется у компании. Доступ перейдёт назначенному водителю. Незавершённое предложение перейдёт в ручное согласование.</p>`,
        async (w) => {
    ...
    async settings() {
      ...
      w.querySelector('#oc-day-override').onclick = () =>
        this.hoursForm({ day: ukDate(), opens: '08:00', closes: '16:00', closed: false }, 'Исключение на дату');
