...
(function () {
  ...
  class Chat {
    ...
    preview() {
      const b = this.$('#oc-body');
      /* «Только дата» работает и в предложении, и в ручном подтверждении: клиенту обещают
         день, а окно прибытия уходит утром при старте маршрута. */
      const dayOnly = !!this.dayOnly && !!this.$('#oc-dayonly');
      for (const id of ['#oc-start', '#oc-end']) {
        const el = this.$(id);
        if (el) el.closest('label').style.display = dayOnly ? 'none' : '';
      }
      if (dayOnly && !this.hoursCache) {
        this.call('hours')
          .then((h) => {
            this.hoursCache = h;
      ...
      if (dayOnly && this.mode === 'confirm') {
        const hc = this.dayHours(this.$('#oc-day').value);
        if (hc) {
          this.$('#oc-start').value = hc.opens;
          this.$('#oc-end').value = hc.closes;
        }
        return;
      }
      if (b && dayOnly) {
   ...
   <textarea id="oc-mv-text" rows="7" readonly></textarea>`,
    ...
    async send() {
      ...
      /* Новое предложение клиенту, который уже подтвердил время. Сервер принимает его
         только с этим признаком: прежний сбор держится до ответа YES. */
      if (this.mode === 'offer' && this.data.address?.date) payload.reoffer = true;
    ...
    async confirm() {
      ...
      /* «Только дата»: клиенту обещан день целиком, точного интервала нет.
         Окно = рабочий день; время прибытия уйдёт утром при старте маршрута. */
      if (this.dayOnly && this.$('#oc-dayonly')) {
        const h = this.dayHours(data.day);
        if (!h) {
          this.notice(T('На эту дату нет рабочих часов — выберите другой день.'));
          return;
        }
      ...
      if (result) {
        this.notice(
          this.dayOnly
            ? T('Дата сохранена. Окно прибытия уйдёт утром при старте маршрута.')
            : T('Дата и время сохранены. Адрес добавлен в маршрут.'),
        );
        await this.loadDetail();
        await this.loadList();
        this.compose();
