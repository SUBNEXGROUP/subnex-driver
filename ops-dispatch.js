...
(function (root) {
  ...
  ${num('reminder_hours', T('Напоминание через, часов'), T('И столько же между следующими'), 1, 96)}
  ${num('reminder_max', T('Напоминаний, не больше'), T('Потом заявка закрывается'), 1, 5)}
  ${num('close_hours', T('Закрыть заявку через, часов'), T('От первого предложения. Раньше всех напоминаний не закроется'), 2, 240)}
